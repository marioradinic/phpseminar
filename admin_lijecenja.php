<?php

include("provjeriadminprava.php");	

if ($_SESSION['user']['roladoktor'] == '0')
{
	header("Location: index.php?pg=7");
}

$act = 1;

if(isset($_GET['act'])) { $act = $_GET['act']; }

if(isset($_POST['act'])) { $act = $_POST['act']; }

?>
<?php
if ($act == 1)
{
$query  = "Select l.*, kor.prezime + ' ' + kor.ime Pacijent, ";
$query .="kor2.prezime + ' ' + kor2.ime Lijecio, mp.naziv Postupak, n.BrojNarudzbe ";
$query .="from Lijecenja l ";
$query .="inner join narudzbe n on l.Narudzba_Id=n.id ";
$query .="inner join korisnici kor on n.Korisnik_Id=kor.id ";
$query .="inner join korisnici kor2 on l.Lijecio_Id=kor2.id ";
$query .="inner join termini t on n.termin_Id=t.id ";
$query .="inner join medicinskipostupci mp on t.MedicinskiPostupak_Id=mp.id";
$params = array();
$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
$result = @sqlsrv_query($MsSQL, $query, $params, $options);	
?>

    <table class="table table-striped">
        <thead>
          <tr>
            <th scope="col" style="width:150px"></th>
            <th scope="col">Broj narudžbe</th>
			<th scope="col">Datum liječenja</th>
			<th scope="col">Pacijent</th>
			<th scope="col">Postupak</th>
			<th scope="col">Liječio</th>
          </tr>
        </thead>
        <tbody>

<?php
while($row = @sqlsrv_fetch_array($result)) {
	
$items_id = $row['Id'];
$items_BrojNarudzbe= $row['BrojNarudzbe'];
$items_DatumLijecenja = $row['DatumLijecenja']->format('d. m. Y');
$items_Pacijent = $row['Pacijent'];
$items_Postupak = $row['Postupak'];
$items_Lijecio = $row['Lijecio'];
?>
          <tr>
            <td>
              <a href="index.php?pg=<?php print $pg ?>&act=2&id=<?php print $items_id ?>" class="btn btn-primary"><span class="glyphicon glyphicon-search"></span></a>
              <a href="index.php?pg=<?php print $pg ?>&act=3&id=<?php print $items_id ?>" class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span></a>
			</td>
			<td><?php print $items_BrojNarudzbe ?></td>
            <td><?php print $items_DatumLijecenja ?></td>
            <td><?php print $items_Pacijent ?></td>
			<td><?php print $items_Postupak ?></td>
			<td><?php print $items_Lijecio ?></td>
          </tr>
<?php
}
?>
        </tbody>
      </table>
	  <a href="index.php?pg=<?php print $pg ?>&act=6" class="btn btn-primary btn-lg">Dodaj novo</a>
<?php
}
else if ($act == 2 && is_numeric($_GET['id']))
{
$query  = "Select l.*, kor.prezime + ' ' + kor.ime Pacijent, ";
$query .="kor2.prezime + ' ' + kor2.ime Lijecio, mp.naziv Postupak, n.BrojNarudzbe ";
$query .="from Lijecenja l ";
$query .="inner join narudzbe n on l.Narudzba_Id=n.id ";
$query .="inner join korisnici kor on n.Korisnik_Id=kor.id ";
$query .="inner join korisnici kor2 on l.Lijecio_Id=kor2.id ";
$query .="inner join termini t on n.termin_Id=t.id ";
$query .="inner join medicinskipostupci mp on t.MedicinskiPostupak_Id=mp.id ";
$query .="where l.Id=" . $_GET['id'];
$params = array();
$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
$result = @sqlsrv_query($MsSQL, $query, $params, $options);
$row = @sqlsrv_fetch_array($result);
$rowcount = sqlsrv_num_rows($result);

if ($rowcount > 0) 
{
$items_id = $row['Id'];
$items_BrojNarudzbe= $row['BrojNarudzbe'];
$items_DatumLijecenja = $row['DatumLijecenja']->format('d. m. Y');
$items_Postupak = $row['Postupak'];
$items_Pacijent = $row['Pacijent'];
$items_OpisLijecenja= $row['OpisLijecenja'];
$items_Lijecio = $row['Lijecio'];
?>
	<div class="details">
	<b>Liječenje:</b><br>
	<label>Id: </label>
	<div><?php print $items_id ?></div>
	<label>Broj narudžbe: </label>
	<div><?php print $items_BrojNarudzbe ?></div>
	<label>Datum liječenja: </label>
	<div><?php print $items_DatumLijecenja ?></div>
	<label>Medicinski postupak: </label>
	<div><?php print $items_Postupak ?></div>
	<label>Pacijent: </label>
	<div><?php print $items_Pacijent ?></div>
	<label>Opis Liječenja: </label>
	<div><?php print $items_OpisLijecenja ?></div>
	<label>Liječio: </label>
	<div><?php print $items_Lijecio ?></div>
	</div>
<?php
}
else{
	print '<div class="msginfo">Ne postoji odabrani zapis!</div>';
}
}
else if ($act == 3 && is_numeric($_GET['id']))
{
	$query  = "Select l.*, kor.prezime + ' ' + kor.ime Pacijent, ";
	$query .="kor2.prezime + ' ' + kor2.ime Lijecio, mp.naziv Postupak, n.BrojNarudzbe ";
	$query .="from Lijecenja l ";
	$query .="inner join narudzbe n on l.Narudzba_Id=n.id ";
	$query .="inner join korisnici kor on n.Korisnik_Id=kor.id ";
	$query .="inner join korisnici kor2 on l.Lijecio_Id=kor2.id ";
	$query .="inner join termini t on n.termin_Id=t.id ";
	$query .="inner join medicinskipostupci mp on t.MedicinskiPostupak_Id=mp.id ";
	$query .="where l.Id=" . $_GET['id'];
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	$result = @sqlsrv_query($MsSQL, $query, $params, $options);
	$row = @sqlsrv_fetch_array($result);
	$rowcount = sqlsrv_num_rows($result);

	if ($rowcount > 0) 
	{
	$items_id = $row['Id'];
	$items_BrojNarudzbe = $row['BrojNarudzbe'];
	$items_OpisLijecenja= $row['OpisLijecenja'];
	$items_Narudzba = $row['Narudzba_Id'];
?>
		<div class="details">
		<b>Liječenje:</b><br>
		<form action="" id="items_form" name="items_form" method="POST" enctype="multipart/form-data">
			<input type="hidden" id="act" name="act" value="4">
			
			<label>Broj narudžbe *</label>
			<input type="text" id="brojNarudzbe" name="brojNarudzbe" value="<?php print $items_BrojNarudzbe ?>" placeholder="Broj narudžbe.." disabled>
			
			<label>Narudžba *</label>
			<select id="narudzba" name="narudzba" disabled>
			<?php 
			$query  = "Select n.*, mp.naziv + ' ' + kor.prezime + ' ' + kor.ime + ' ' + format(t.datumtermina,'(dd.MM.yyyy)') Narudzba ";
			$query .="from narudzbe n ";
			$query .="left join Lijecenja l on l.Narudzba_Id=n.id ";
			$query .="inner join termini t on t.id=n.Termin_id ";
			$query .="inner join medicinskipostupci mp on t.MedicinskiPostupak_Id=mp.id ";
			$query .="inner join korisnici kor on n.Korisnik_Id=kor.id ";
			$query .="where n.id=" . $items_Narudzba;
			$query .=" order by mp.naziv, DatumTermina asc ";
			$result = @sqlsrv_query($MsSQL, $query);
			while($row = @sqlsrv_fetch_array($result)) {
			$selected='';
			if ($row['Id'] == $items_Narudzba)
			{
				$selected='selected';
			}
			print '<option ' . $selected . ' value="' . $row['Id'] . '">' . $row['Narudzba'] . '</option>';
			}
			?>
			</select>

			<label>Opis Liječenja *</label>
			<textarea id="opisLijecenja" name="opisLijecenja" rows="2" cols="20" required=""><?php print $items_OpisLijecenja ?></textarea>
			<hr>
			
			<input type="submit" class="btn btn-primary btn-lg" value="Izmijeni">
		</form>
		</div>
<?php
}
else{
	print '<div class="msginfo">Ne postoji odabrani zapis!</div>';
}
}
else if ($act == 4)
{
$query  = "SELECT * FROM Lijecenja";
$query .= " WHERE id=" . $_GET['id'];
$params = array();
$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
$result = @sqlsrv_query($MsSQL, $query, $params, $options);
$row = @sqlsrv_fetch_array($result);
$rowcount = sqlsrv_num_rows($result);
if ($rowcount > 0) 
{
$query  = "UPDATE Lijecenja SET OpisLijecenja=(?)";
$query .= " WHERE id=" . $_GET['id'];
$params = array($_POST['opisLijecenja']);
$result = sqlsrv_query($MsSQL, $query, $params);

$items_id = $_GET['id'];

print '<div class="msginfo">Ažuriran je zapis id: '. $items_id .'.</div>';		
}
else{
	print '<div class="msginfo">Ne postoji odabrani zapis!</div>';
}
}
else if ($act == 6)
{
	$query  = "Select t.*, mp.naziv + ' ' + b.naziv + ' ' + format(t.datumtermina,'(dd.MM.yyyy)') Termin from termini t ";
	$query .="inner join bolnice b on t.Bolnica_Id=b.id ";
	$query .="inner join medicinskipostupci mp on t.MedicinskiPostupak_Id=mp.id ";
	$query .="left join narudzbe n on t.id=n.Termin_id ";
	$query .="left join statusinarudzbe st on n.status_id=st.id ";
	$query .="where DatumTermina>GETDATE() and (n.id is null or n.status_id=2) ";
	$query .="order by mp.naziv, DatumTermina asc ";
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	$resulttermini = @sqlsrv_query($MsSQL, $query, $params, $options);
	$rowcount = sqlsrv_num_rows($resulttermini);

	if ($rowcount == 0)
	{
		print '<div class="msginfo">Nema aktivnih termina!</div>';
	}
	else
	{
	?>	
	<div class="details">
		<b>Liječenje:</b><br>
		<form action="" id="items_form" name="items_form" method="POST" enctype="multipart/form-data">
			<input type="hidden" id="act" name="act" value="7">

			<label>Narudžba *</label>
			<select id="narudzba" name="narudzba">
			<?php 
			$query  = "Select n.*, mp.naziv + ' ' + kor.prezime + ' ' + kor.ime + ' ' + format(t.datumtermina,'(dd.MM.yyyy)') Narudzba ";
			$query .="from narudzbe n ";
			$query .="left join Lijecenja l on l.Narudzba_Id=n.id ";
			$query .="inner join termini t on t.id=n.Termin_id ";
			$query .="inner join medicinskipostupci mp on t.MedicinskiPostupak_Id=mp.id ";
			$query .="inner join korisnici kor on n.Korisnik_Id=kor.id ";
			$query .="where l.id is null ";
			$query .="order by mp.naziv, DatumTermina asc ";
			$result = @sqlsrv_query($MsSQL, $query);
			while($row = @sqlsrv_fetch_array($result)) {
			$selected='';
			if ($row['Id'] == $items_Narudzba)
			{
				$selected='selected';
			}
			print '<option ' . $selected . ' value="' . $row['Id'] . '">' . $row['Narudzba'] . '</option>';
			}
			?>
			</select>
			
			<label>Opis Liječenja *</label>
			<textarea id="opisLijecenja" name="opisLijecenja" rows="2" cols="20" required=""></textarea>

			<hr>
			
			<input type="submit" class="btn btn-primary btn-lg" value="Dodaj">
		</form>
		</div>
	<?php
	}
}
else if ($act == 7)
{
	$query  = "INSERT INTO Lijecenja (Narudzba_Id, Lijecio_Id, OpisLijecenja)";
	$query .= " VALUES (?, ?, ?); SELECT SCOPE_IDENTITY()";
	$params = array($_POST['narudzba'],$_SESSION['user']['id'], $_POST['opisLijecenja'], );
	$result = sqlsrv_query($MsSQL, $query, $params);
	sqlsrv_next_result($result); 
	sqlsrv_fetch($result); 
	$id=sqlsrv_get_field($result, 0); 

	$query  = "UPDATE Narudzbe SET Status_Id=(?)";
	$query .= " WHERE id=" . $_POST['narudzba'];
	$params = array(3);
	$result = sqlsrv_query($MsSQL, $query, $params);

	$query  = "update lijecenja set DatumLijecenja=DatumTermina from Narudzbe nar inner join Termini ter on nar.Termin_Id=ter.Id";
	$query .= " where nar.id=".$id." and Lijecenja.Narudzba_Id=" . $id;
	$result = sqlsrv_query($MsSQL, $query);

	print '<div class="msginfo">Dodan je novi zapis.</div>';	
}
?>	
<?php
if ($act != 1)
{
?>	
<div style="margin-top:20px;padding-left:15px"><a href="index.php?pg=<?php print $pg ?>&act=1">Natrag</a></div>
<?php
}
?>